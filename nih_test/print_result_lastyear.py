...
Get the top 5 words of the last year
...
plot_prefix = data_prefix + 'wfrom2013/'
...
for yid in range(num_timeslices)[::-1]:
    print('--- time: ' + str(yid))
...
#labels = cp.load(open('/home/jihyunp/research/topicmodel/data/150K/label_names.pkl','r'))
#for i in range(1,11):
#    labels.append('BG'+ str(i))
...
for tid in range(total_num_topics):
    ...
    for word in wlist:
        ...
        problist = np.array(topic_wordpatterns[tid][word][::-1])
        line, = axes.plot(problist,  'o-', label=word)
        linelist.append(line)
    axes.legend(handles=linelist)
    axes.set_xticklabels(year_label)
    axes.set_title('Word Probabilities for Topic '+ str(tid))
    # axes.set_title('Word Probabilities (Topic '+ str(tid) +': '+ labels[tid]+')')
...
for tid in range(total_num_topics):
    ...
    for word in wlist:
        # problist = topic_wordpatterns[tid][word]
        problist = np.array(marginal_wordpatterns[word][::-1])
        line, = axes.plot(problist,  'o-', label=word)
        linelist.append(line)
    axes.legend(handles=linelist)
    axes.set_xticklabels(year_label)
    axes.set_title('Marginal Word Probabilities')
    #axes.set_title('Marginal probabilities (Topic '+ str(tid) +': '+ labels[tid]+')')
    axes.set_xlim([0, num_timeslices-1])
...
for tid in range(total_num_topics):
    ...
    for word in wlist:
        # problist = topic_wordpatterns[tid][word]
        problist = np.array(topic_wordpatterns[tid][word][::-1]) / np.array(marginal_wordpatterns[word][::-1]) / total_num_topics
        line, = axes.plot(problist,  'o-', label=word)
        linelist.append(line)
    axes.legend(handles=linelist)
    axes.set_xticklabels(year_label)
